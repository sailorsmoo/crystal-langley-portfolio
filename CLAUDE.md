...
- The click/keydown handler is duplicated in the homepage's inline `<script>` and in `/telltale/index.html`'s own inline `<script>` (small enough that it wasn't worth promoting into the shared `case-study.js` for a single consumer).
