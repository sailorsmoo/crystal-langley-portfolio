...
## Motion / Telltale Games sections (homepage)

- `#motion` — unnumbered section (kicker "—") right after Capabilities, before About. A loose grid of 6 personal reel videos (motion graphics, game UI/UX demos, a B2B animation), deliberately not tied to any case study.
- `#telltale` — unnumbered section right after Motion, before About. 12 Vimeo clips grouped under 4 game headings (The Wolf Among Us, Poker Night, Game of Thrones, Tales from the Borderlands) — this is the Telltale body of work referenced by the "Telltale Games" work tile (marked "In Progress," no dedicated case study yet).
- Both use the same `.motion__item` component: a click/keyboard-activated facade card (real YouTube thumbnail via `img.youtube.com/vi/{id}/hqdefault.jpg` where the source is YouTube; a plain ink-colored branded card for Vimeo, since Vimeo has no predictable direct thumbnail URL and fetching one via oEmbed needs a network call this environment can't reliably make). Clicking/Enter/Space swaps the card's content for a real `<iframe>` embed with `autoplay=1` — nothing loads an iframe on page load, which matters given the homepage's Lighthouse Performance score is already only 76/100.
- Shared JS lives in the homepage's own inline `<script>` (`.motion__item` click/keydown handlers) — if this pattern gets reused on a case-study page later, port the same handler into `case-study.js` rather than duplicating it ad hoc.
